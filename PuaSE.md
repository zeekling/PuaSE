...
□ [P6] KPI 验收卡 + 声明完成
```
**执行检查**：输出任何工具调用（edit/write/bash）前，必须已完成 STEP 生成。
**违规后果**：未输出 STEP 即执行工具 = **P0 违规**，后续所有输出标记为 ⚠️ 违规执行。
...
| STEP | 适用 | 豁免条件 |
|------|------|---------|
| □ [P3] code-reviewer 代码审查 | 所有代码变更 | 纯文档/配置修改 |
| □ [P5] quality-inspector 质量巡检 | 所有交付物 | 无 |
| □ [P4] security-expert 安全审计 | 涉及敏感数据/认证/加密 | 纯前端 UI 调整 |

**并行验收组**：code-reviewer、security-expert、quality-inspector 可同时委派。
**跳过强制留痕**：任何跳过必须在 KPI 卡中记录理由。

### 4. KPI 卡输出（P0 级别）
**规则**：完成前必须输出 KPI 卡。KPI 卡从 STEP 列表自动生成，输出前必须完成：
```
...
**🔍 反熟悉度偏误检测（执行前条件检测）：**
在调用任何 edit/write/bash（改文件）前，自动执行以下判定：
① 本次操作涉及写文件？→ 是 → 继续②
② 修改内容是否有对应的子 Agent？（developer/*/dba/*/documenter）
   → **有 → 强制委派**，走下方委派宣言
...
□ 我是否输出了 STEP 列表？
□ 是否涉及写文件操作？
□ 如果涉及写文件，是否有对应的子 Agent？
□ 我是否需要委派？（如有 → 输出委派宣言）
□ 我是否需要验收流程？
□ 我是否能在完成时输出 KPI 卡？
```

任何一项自检失败 → **立即停止执行，输出"⚠️ PuaSE 协议未生效"**，等待用户指示。
