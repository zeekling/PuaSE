...
| ✅ **影响面清单** | 修改前 | 漏了 = 线上炸了 |
| ✅ **复盘清单** | 见 §4.2 执行规则中的复盘要求（默认不启动，subagent 被多次打回时触发） | 不复盘 = 下次还踩坑 |
...
- **复盘闭环**：参见 §4.2 执行规则——默认不启动 reflector，仅当 subagent 被多次打回时委派 reflector 进行复盘。纯文档/配置修改无需复盘。
...
- 全部通过 → 继续
- **复盘规则**：默认不启动复盘。仅当同一 subagent 在同一任务中被多次打回（≥2 次）时，才需委派 reflector 进行复盘反思。复盘需手动总结，复盘完成是 KPI 卡输出的前置条件。
...
□ reflector（仅当 subagent 被多次打回时触发——当前打回次数: N）
...
> 7. ✅ 复盘已完成（如触发）——按 §4.2 执行规则复盘要求完成（手动总结）；未触发则标记"默认跳过"
...
□ **复盘（条件触发）** — 按 §4.2 执行规则复盘要求完成
  → 仅当 subagent 被多次打回（≥2 次）时触发 reflector
  → 未触发复盘 → KPI 卡标记"默认跳过（未触发多轮打回）"
  → 已触发但未完成 → 不得输出 KPI 卡
...
- 复盘反思仅在 subagent 被多次打回（≥2 次）时触发，已触发但未完成复盘不得输出 KPI 卡
