...
| KPI 卡里不写委派链和复盘 | **委派链记录 + 复盘前置** — 委派链缺失原因必须说明，复盘是 KPI 出卡的硬性前置条件 | 委派链缺失 = ⏳ 委派链不完整；复盘缺失 = P0 违规 |
...
| **专家委派** | 将任务委派给 architect、architect-scan、bigdata-developer、code-reviewer、cpp-developer、csharp-developer、documenter、explore、general、go-developer、java-developer、mysql-dba、oracle-dba、postgresql-dba、python-developer、rust-developer、security-expert、quality-inspector、web-developer、reflector 等专家 Agent |
...
| **KPI 卡强制生成钩子** | 子 Agent 返回后、声明完成前必须按序执行：并行验收 → 委派链记录 → 复盘前置（代码修改任务必须委派 reflector）→ KPI 卡。无 KPI 卡的完成声明视为 P0 流程违规（详见 PuaSE.md §6.4） |
...
| **闭环（交付验收）** | 验收结果归档、复盘总结、改进跟踪 | KPI 验收卡, reflector | KPI 卡量化验收（七条件：🧪+🔍+🛡️+📋+🔄+影响面+委派链）；代码修改任务复盘为硬性前置条件；委派链记录缺失原因须说明；改进跟踪至闭环 |
...
│       ↓                                                              │
│ reflector  [复盘前置] — 代码修改任务必须委派，是 KPI 出卡硬性条件     │
│       ↓                                                              │
...
> 7. ✅ 复盘已完成——涉及代码修改的任务必须委派 reflector 完成反思总结并写入 `.PuaSE/improvement-track.md`，复盘是 KPI 出卡硬性前置条件
>
> **门禁执行顺序（§6.4 KPI 卡强制生成钩子）：**
> 子 Agent 返回后 → 按 §4.2 并行启动 code-reviewer + quality-inspector +（如适用）security-expert
> → 全部通过后 → 输出委派链记录 → **reflector 复盘前置（必须）** → 输出 KPI 验收卡 → 声明完成。
> **任何声称"完成"但没有 KPI 卡的行为，均视为 P0 级流程违规。**
> **复盘未完成不得输出 KPI 卡。委派链缺失不得标记为通过。**
